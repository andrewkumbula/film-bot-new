...
**Дозаполнение kinopoisk_id:** фильмы, которые ИИ рекомендовал, но Кинопоиск не нашёл (например из‑за «е» вместо «ё»), сохраняются в таблицу `movies` без `kinopoisk_id`. Раз в сутки в 03:30 запускается задача: для таких записей выполняется поиск в интернете через **Tavily** по запросу «название фильма год кинопоиск», результаты передаются в ИИ, который извлекает из них точное название (как на Кинопоиске), затем выполняется повторный запрос в API Кинопоиска. Так ИИ опирается на реальные результаты поиска, а не на свои знания. Нужен `TAVILY_API_KEY` (ключ на [tavily.com](https://tavily.com)); без него уточнение названия не выполняется, используется исходное. Ручной запуск: `python scripts/run_kinopoisk_backfill.py [лимит]`.
